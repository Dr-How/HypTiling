...
// Using the Poincare disk model where the unit disk represents hyperbolic space
// 使用Poincare圆盘模型，其中单位圆盘表示双曲空间
// ============================================================================

// Calculate hyperbolic distance between two points in the Poincare disk model
// The distance formula is: d(z1,z2) = 2*log((|z1-z2| + sqrt(|z1|²|z2|² - 2<z1,z2> + 1))/sqrt((1-|z1|²)(1-|z2|²)))
// 计算Poincare圆盘模型中两点之间的双曲距离
...
// This uses the formula for distance to a geodesic in the Poincare disk model
// 计算点z到z1和z2之间测地线的双曲距离
// 使用Poincare圆盘模型中点到测地线距离的公式
...
// Find the hyperbolic midpoint between two points in the Poincare disk model
// The midpoint is the point that is equidistant from both z1 and z2
// 在Poincare圆盘模型中找到两点之间的双曲中点
// 中点是到z1和z2距离相等的点
